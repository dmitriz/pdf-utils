...
const mergePdfsFromDisk = async ({ pdfPaths, outputPath, allowOutsideBaseDir = false }) => {
  try {
    // Ensure pdfPaths is iterable
    if (!pdfPaths) {
      throw new Error('pdfPaths must be an array');
    }
    ...
    // Merge the PDF buffers - use the module's exported function
    // so it can be properly spied on in tests
    const mergedPdfBuffer = await module.exports.mergePdfsInMemory(pdfBuffers);
